...
import { getUser, requireAdmin, requireUser } from '../../../utils';
...
const createNewComment = catchAsync(async (req, res) => {
  ...
  if (!(await Blog.findById(data.belongsTo))) {
    throw new Error('Post to which this comment belongs to not found');
  }
